...
<script type="text/javascript">
	$(function() {
		...
		$("#all").click(function() {
			if ($(this).is(":checked")) {
				$('[name="fgids"]').prop("checked", true);
			} else {
				$('[name="fgids"]').prop("checked", false);
			}
		});
		//批量修改
		$("#approvedMany").click(function() {
			var ch = $('[name="fgids"]:checked'); //被选中的元素
			if (ch.length == 0) {//长度为0说明没有被选中元素 
				alert("请至少选中一个要修改的元素!");
				return;//代码不往下执行 
			}//有被选中的元素 
			var url = "approvedManyFgLogin.do?fgids=";//基础url
	...
	$(function() {
		...
		$("#notapprovedMany").click(function() {
			...
			var url = "notapprovedManyFgLogin.do?fgids=";//基础url
			for (var i = 0; i < ch.length; i++) {
				url += $(ch[i]).val() + ",";
			}
			window.location.href = url;//发送请求
...
<form action="toadminfgLogin.do" method="post">
	<p>
...
<div class="row">
	<c:forEach items="${all}" var="a">
		<div class="col-lg-5">
		<input type="checkbox" name="fgids" value="${a.fgid}" />
		...
		<div class="col-lg-7">
			<div class="product-details-des">
				...
				<p class="pro-desc">捡到地点：${a.place}</p>
				<p class="pro-desc">
					时间：
			...
			<a href="adminapprovedfgLogin.do?fgid=${a.fgid}"
				onclick="javascript:return app()">审核通过</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a href="adminnotapprovedfgLogin.do?fgid=${a.fgid}"
...
<form>
	...
	<center>
		<p style="font-family: \5FAE\8F6F\96C5\9ED1; font-size: 200%">
			<a
				href="toadminfgLogin.do?pno=${page.navigateFirstPage}&&goodname=${goodname}">首页</a>&nbsp;
			<a
				href="toadminfgLogin.do?pno=${page.hasNextPage ? page.nextPage:page.navigateLastPage}&&goodname=${goodname}">下页</a>&nbsp;
			<a
				href="toadminfgLogin.do?pno=${page.hasPreviousPage ? page.prePage:page.navigateFirstPage}&&goodname=${goodname}">上页</a>&nbsp;
			<a
				href="toadminfgLogin.do?pno=${page.navigateLastPage}&&goodname=${goodname}">末页</a>&nbsp;当前第${page.pageNum}页/共${page.pages}页
		</p>
	</center>
</form>
		...
		<div class="offcanvas-search-inner">
			...
			<div class="container">
				<div class="offcanvas-search-box">
					<center><form action="toadminfgLogin.do" class="d-flex bdr-bottom w-100" method="post">
